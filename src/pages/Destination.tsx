      ...
      <div className="">
        ...
        <div className="content">
          <div className="contentTabs">
            <div className="d-flex justify-content-evenly align-items-center w-60">
              {destData.map((d,i)=>(
                <p className={i==num ? 'active':""} onClick={()=>setNum(i)} key={i}>{d.name}</p>
              ))}
            </div>
          </div>
          <div className="name">{dest.name}</div>
          <div className="description">{dest.description}
          </div>
          <hr />
